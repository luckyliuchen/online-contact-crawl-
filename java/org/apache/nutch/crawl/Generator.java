...
public class Generator extends Configured implements Tool {
  ...
  public static class Selector implements
    ...
    public void configure(JobConf job) {
        try{
            LOG.info("Selector configure input path = " + FileInputFormat.getInputPaths(job)[0].toString() );
            LOG.info("Selector configure out path = " + FileOutputFormat.getOutputPath(job).toString() );
        	
        }catch( Exception e ){
        	LOG.info( " Generator generate INput output error " + e.getMessage() );
        	e.printStackTrace();
        }
    ...
    public void map(Text key, CrawlDatum value,
        ...
        throws IOException {
      Text url = key;
      LOG.info("generator Selector Map url = " + url.toString());
      if (filter) {
      ...
      LOG.info(" Selector Map sortValue = " + sortValue + " ; entry = " + entry );
    ...
    public void reduce(FloatWritable key, Iterator<SelectorEntry> values,
       ...
       while (values.hasNext()) {
        ...
        LOG.info(" Selector reduce  entry = " + entry );
       Text url = entry.url;
       LOG.info(" Selector reduce url = " + url.toString());
  ...
  public static class CrawlDbUpdater extends MapReduceBase implements
    ...
    public void configure(JobConf job) {
      ...
      LOG.info("CrawlDbUpdater configure input path = " + FileInputFormat.getInputPaths(job)[0].toString() );
      LOG.info("CrawlDbUpdater configure out path = " + FileOutputFormat.getOutputPath(job).toString() );
    }
  ...
  public Path[] generate(Path dbDir, Path segments, int numLists, long topN,
    ...
    LOG.info("Generator generate 1111 start ");
    ...
    LOG.info("Generator generate 1111 end ");
    ...
    LOG.info("Generator generate generatedSegments 222 end ");
    ...
    LOG.info("Generator generate generatedSegments 3333 next " + getConf().getBoolean(GENERATE_UPDATE_CRAWLDB, false) );
