...
public class Crawl extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    LOG.info( " Crawl Start ,  depth = " + depth );
    int i;
    for (i = 0; i < depth; i++) {             // generate new segment
      Path[] segs = generator.generate(crawlDb, segments, -1, topN, System
          .currentTimeMillis());
      LOG.info( " Crawl Start ,  generate End segs.length  = " + segs.length + " ; " + segs.toString() );
      if (segs == null) {
        LOG.info("Stopping at depth=" + i + " - no more URLs to fetch.");
        break;
      }
      fetcher.fetch(segs[0], threads);  // fetch it
      LOG.info( " Crawl Start ,  fetcher End "  );
      if (!Fetcher.isParsing(job)) {
        parseSegment.parse(segs[0]);    // parse it, if needed
      }
      crawlDbTool.update(crawlDb, segs, true, true); // update crawldb
      LOG.info( " Crawl Start ,  update End "  );
    }
    
    if (i > 0) {
      linkDbTool.invert(linkDb, segments, true, true, false); // invert links

      LOG.info( " Crawl Start ,  inkDbTool.invert End "  );
