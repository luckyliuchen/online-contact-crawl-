...
public class Fetcher extends Configured implements Tool,
    ...
    public void run() {
      ...
      while (hasMore) {
        if (System.currentTimeMillis() >= timelimit && timelimit != -1) {
    	  ...
    	  LOG.info("QueueFeeder run size = " + size + " ; queues.getTotalSize() = " + queues.getTotalSize() );
        ...
        if (feed <= 0) {
          ...
          while (feed > 0 && hasMore) {
            try {
              Text url = new Text();
              CrawlDatum datum = new CrawlDatum();
          	  LOG.info("QueueFeeder run 222 url = " + url + " ; datum = " + datum.toString() );
             hasMore = reader.next(url, datum);
              if (hasMore) {
  ...
  private class FetcherThread extends Thread {
        ...
        while (true) {
          ...
          try {
            ...
            do {
              ...
              LOG.info(" FetcherThread run protocol = " + protocol.toString() + " ; rules = " + rules.toString() );
              ...
              LOG.info(" FetcherThread run status.getCode() = " + status.getCode() + " ; urlString = " + urlString + " ; content = " + content.toString() );
    ...
    private ParseStatus output(Text key, CrawlDatum datum,
      ...
      if (content != null) {
         ...
         * original URL. */ 
        LOG.info("FetcherThread output = " + output.toString() + " parsing =" + parsing + " ; status " + status + " CrawlDatum.STATUS_FETCH_SUCCESS = " + CrawlDatum.STATUS_FETCH_SUCCESS );
      ...
      try {
        ...
        LOG.info("FetcherThread parseResult = " + parseResult.toString()  );
        if (parseResult != null) {
  ...
  public void configure(JobConf job) {
    ...
    LOG.info("Fetcher configure input path = " + FileInputFormat.getInputPaths(job)[0].toString() );
    LOG.info("Fetcher configure out path = " + FileOutputFormat.getOutputPath(job).toString() );
...
//    }
  ...
  public void run(RecordReader<Text, CrawlDatum> input,
    ...
    LOG.info(" Fetcher Run  threadCount " + threadCount );
